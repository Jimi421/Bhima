...
def interactive_config():
    ...
    url = input(Fore.WHITE + "Target URL (leave blank if using CIDR): " + Style.RESET_ALL).strip()
    cidr = input(Fore.WHITE + "CIDR block (optional): " + Style.RESET_ALL).strip()
    while not url and not cidr:
        print(Fore.YELLOW + "A URL or CIDR block is required." + Style.RESET_ALL)
        url = input(Fore.WHITE + "Target URL (leave blank if using CIDR): " + Style.RESET_ALL).strip()
        cidr = input(Fore.WHITE + "CIDR block (optional): " + Style.RESET_ALL).strip()
    quick = input(Fore.WHITE + "Run with defaults for all other settings? [Y/n]: " + Style.RESET_ALL).strip().lower()
    if quick in ('', 'y', 'yes'):
        summary = {'url': url, 'cidr': cidr}
        summary.update(defaults)
        sc = input(Fore.WHITE + f"Scheme [{defaults['scheme']}]: " + Style.RESET_ALL).strip() or defaults['scheme']
        pt = input(Fore.WHITE + f"Port [{defaults['port']}]: " + Style.RESET_ALL).strip()
        summary['scheme'] = sc
        summary['port'] = int(pt) if pt.isdigit() else defaults['port']
        print(Fore.CYAN+"\nUsing defaults for other settings:"+Style.RESET_ALL)
        for k,v in summary.items():
            flag = Fore.GREEN+" (default)"+Style.RESET_ALL if k not in ('url','cidr','scheme','port') else ""
    ...
    un = input(Fore.WHITE + f"Username [{defaults['username']}]: " + Style.RESET_ALL).strip() or defaults['username']
    pw = input(Fore.WHITE + f"Password [{defaults['password']}]: " + Style.RESET_ALL).strip() or defaults['password']
    sc = input(Fore.WHITE + f"Scheme (http/https) [{defaults['scheme']}]: " + Style.RESET_ALL).strip().lower() or defaults['scheme']
    pt = input(Fore.WHITE + f"Port [{defaults['port']}]: " + Style.RESET_ALL).strip()
    port = int(pt) if pt.isdigit() else defaults['port']
    od = input(Fore.WHITE + f"OOB Domain [{defaults['oob_domain']}]: " + Style.RESET_ALL).strip() or defaults['oob_domain']
    summary = {'url':url,'cidr':cidr,'wordlist':wl,'output':out,'threads':threads,'status':st,'format':fmt,'proxy':px,'profile':profile,'bypass_403':bypass_403,'login_url':lu,'username':un,'password':pw,'oob_domain':od,'scheme':sc,'port':port}
